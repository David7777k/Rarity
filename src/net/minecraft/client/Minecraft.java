...
import im.rarity.events.EventChangeWorld;
import im.rarity.events.TickEvent;
...
import im.rarity.functions.impl.player.ItemCooldown;
import im.rarity.ui.mainmenu.MainScreen;
...
public class Minecraft extends RecursiveEventLoop<Runnable> implements ISnooperInfo, IWindowEventListener {
    ...
    public Minecraft(GameConfiguration gameConfig) {
        ...
        SocialInteractionsService field_244734_au = this.func_244735_a(yggdrasilauthenticationservice, gameConfig);
        ...
        this.itemRenderer = new ItemRenderer(this.textureManager, this.modelManager, itemColors);
    ...
    public void setDefaultMinecraftTitle() {
        ...
        if (selfDestruct.unhooked) {
            ...
            this.mainWindow.setWindowTitle("RarityClient — лучший DLC для Minecraft!");
        }
    }
    ...
    private void populateSearchTreeManager() {
        SearchTree<ItemStack> searchtree = new SearchTree<>((stack) -> stack.getTooltip(null, ITooltipFlag.TooltipFlags.NORMAL).stream()
                .map((textComponent) -> Objects.requireNonNull(TextFormatting.getTextWithoutFormattingCodes(textComponent.getString())).trim()).filter((name) -> !name.isEmpty()), (stack) -> Stream.of(Registry.ITEM.getKey(stack.getItem())));
        SearchTreeReloadable<ItemStack> searchtreereloadable = new SearchTreeReloadable<>((stack) -> ItemTags.getCollection().getOwningTags(stack.getItem()).stream());
        ...
        SearchTree<RecipeList> searchtree1 = new SearchTree<>((recipeList) -> recipeList.getRecipes().stream().flatMap((recipe) -> recipe.getRecipeOutput().getTooltip(null, ITooltipFlag.TooltipFlags.NORMAL)
                .stream()).map((textComponent) -> Objects.requireNonNull(TextFormatting.getTextWithoutFormattingCodes(textComponent.getString())).trim()).filter((name) -> !name.isEmpty()), (recipeList) -> recipeList.getRecipes().stream().map((recipe) -> Registry.ITEM.getKey(recipe.getRecipeOutput().getItem())));
        ...
        if (this.futureRefreshResources != null) {
            ...
            if (this.loadingGui instanceof ResourceLoadProgressGui) {
                ...
                this.setLoadingGui(new ResourceLoadProgressGui(this, this.resourceManager.reloadResources(
                        Util.getServerExecutor(), this, RESOURCE_RELOAD_INIT_TASK, list), (throwable) -> Util.acceptOrElse(throwable, this::restoreResourcePacks, () -> {
    ...
    public void displayGuiScreen(@Nullable Screen guiScreenIn) {
        ...
        } else if (guiScreenIn == null && player.getShouldBeDead()) {
            if (player.isShowDeathScreen()) {
    ...
    private void runGameLoop(boolean renderWorldIn) {
        ...
        boolean flag = this.isSingleplayer()
                ...
                && !Objects.requireNonNull(this.integratedServer).getPublic();
    ...
    private void tick(boolean isDebug, @Nullable LongTickDetector detector) {
        Rarity.getInstance().getEventBus().post(tickEvent);
    ...
    private void func_238183_a_(MatrixStack matrixStack, IProfileResult profilerResult) {
        ...
        RenderSystem.ortho(0.0D, this.mainWindow.getFramebufferWidth(),
                this.mainWindow.getFramebufferHeight(), 0.0D, 1000.0D, 3000.0D);
        ...
        bufferbuilder.pos((float) j - 176.0F, (float) k - 96.0F - 16.0F, 0.0D).color(200, 0, 0, 0)
                .endVertex();
        ...
        for (DataPoint datapoint1 : list) {
            ...
            for (int i2 = l; i2 >= 0; --i2) {
                ...
                bufferbuilder.pos((float) j + f1, (float) k - f2, 0.0D).color(j1, k1, l1, 255)
                        .endVertex();
            }
            ...
            for (int l2 = l; l2 >= 0; --l2) {
                ...
                if (!(f5 > 0.0F)) {
                    bufferbuilder.pos((float) j + f4, (float) k - f5, 0.0D)
                            .color(j1 >> 1, k1 >> 1, l1 >> 1, 255).endVertex();
                    bufferbuilder.pos((float) j + f4, (float) k - f5 + 10.0F, 0.0D)
                            .color(j1 >> 1, k1 >> 1, l1 >> 1, 255).endVertex();
                }
            }
    ...
    public void displayInGameMenu(boolean pauseOnly) {
        if (this.currentScreen == null) {
            boolean flag = this.isSingleplayer() && !Objects.requireNonNull(this.integratedServer).getPublic();
        ...
        if (this.leftClickCounter <= 0) {
            if (this.objectMouseOver == null) {
                ...
                switch (this.objectMouseOver.getType()) {
                    case ENTITY:
                        AttackOrder.sendFixedAttack(player, ((EntityRayTraceResult) this.objectMouseOver).getEntity(), Hand.MAIN_HAND);
                ...
                for (Hand hand : Hand.values()) {
                    ...
                    if (this.objectMouseOver != null) {
                        switch (this.objectMouseOver.getType()) {
                            case ENTITY -> {
                                ...
                                ActionResultType actionresulttype = this.playerController.interactWithEntity(player, entity, entityraytraceresult, hand);
                                ...
                                if (!killAuraIsStateAndTargetNotNull) {
                                    if (!actionresulttype.isSuccessOrConsume()) {
                                        actionresulttype = this.playerController.interactWithEntity(player, entity, hand);
                                    }
                                    if (actionresulttype.isSuccessOrConsume()) {
                            ...
                            case BLOCK -> {
                                ...
                                ActionResultType actionresulttype1 = this.playerController.processRightClickBlock(player, this.world, hand, blockraytraceresult);
                    ...
                    if (!itemstack.isEmpty()) {
                        ...
                        if (itemCooldown.isState() && itemEnum != null && itemCooldown.isCurrentItem(itemEnum) && player.getCooldownTracker().hasCooldown(itemEnum.getItem())) {
                            ...
                            actionresulttype2 = this.playerController.processRightClick(player, this.world, hand);
                        }
    ...
    public void runTick() {
        ...
        if (this.currentScreen == null && player != null) {
            if (player.getShouldBeDead() && !(this.currentScreen instanceof DeathScreen)) {
                this.displayGuiScreen(null);
            } else if (player.isSleeping() && this.world != null) {
        ...
        if (this.world != null) {
            assert player != null;
            Rarity.getInstance().getEventBus().post(player.tick);
        ...
        if (this.world != null) {
            ...
            if (!this.isGamePaused && this.world != null) {
                this.world.animateTick(MathHelper.floor(player.getPosX()), MathHelper.floor(player.getPosY()),
                        MathHelper.floor(player.getPosZ()));
            }
    ...
    private void processKeyBinds() {
        ...
        while (this.gameSettings.field_244602_au.isPressed()) {
            if (!this.func_244600_aM()) {
                player.sendStatusMessage(field_244596_I, true);
        ...
        while (this.gameSettings.keyBindInventory.isPressed()) {
            if (this.playerController.isRidingHorse()) {
                ...
                this.displayGuiScreen(new InventoryScreen(player));
            }
        }

        while (this.gameSettings.keyBindAdvancements.isPressed()) {
            this.displayGuiScreen(new AdvancementsScreen(player.connection.getAdvancementManager()));
        }

        while (this.gameSettings.keyBindSwapHands.isPressed()) {
            if (!player.isSpectator()) {
                Objects.requireNonNull(this.getConnection()).sendPacket(new CPlayerDiggingPacket(
    ...
    public void createWorld(String worldName, WorldSettings worldSettings, DynamicRegistries.Impl dynamicRegistriesIn,
        ...
        this.loadWorld(worldName, dynamicRegistriesIn, (worldStorage) -> worldSettings.getDatapackCodec(), (worldStorage, dynamicRegistries, resourceManager, datapackCodec) -> {
            ...
            DataResult<DimensionGeneratorSettings> dataresult = DimensionGeneratorSettings.field_236201_a_
                    ...
                    .flatMap((p_243209_1_) -> DimensionGeneratorSettings.field_236201_a_.parse(worldsettingsimport, p_243209_1_));
    ...
    private void loadWorld(String worldName, DynamicRegistries.Impl dynamicRegistries,
        ...
        try {
            ...
            this.displayGuiScreen(new DatapackFailureScreen(() -> this.loadWorld(worldName, dynamicRegistries, levelSaveToDatapackFunction, quadFunction, true,
                    selectionType)));
        ...
        if (selectionType == WorldSelectionType.NONE || !flag && !flag1) {
            ...
            try {
                ...
                this.integratedServer = MinecraftServer.func_240784_a_((thread) -> new IntegratedServer(thread, this, dynamicRegistries, saveformat$levelsave,
            ...
            this.profiler.endSection();
            assert this.integratedServer.getNetworkSystem() != null;
            SocketAddress socketaddress = this.integratedServer.getNetworkSystem().addLocalEndpoint();
            NetworkManager networkmanager = NetworkManager.provideLocalClient(socketaddress);
            networkmanager
                    .setNetHandler(new ClientLoginNetHandler(networkmanager, this, null, (statusMessage) -> {
                    }));
            ...
            this.deleteWorld(selectionType, worldName, flag, () -> this.loadWorld(worldName, dynamicRegistries, levelSaveToDatapackFunction, quadFunction, vanillaOnly,
    ...
    public void loadWorld(ClientWorld worldClientIn) {
        ...
        Rarity.getInstance().getEventBus().post(new EventChangeWorld());
    }
        ...
        if (this.objectMouseOver != null && this.objectMouseOver.getType() != RayTraceResult.Type.MISS) {
            ...
            if (itemstack.isEmpty()) {
                ...
                if (flag) {
                    ...
                    this.playerController.sendSlotPacket(player.getHeldItem(Hand.MAIN_HAND),
    ...
    public BackgroundMusicSelector getBackgroundMusicSelector() {
        if (this.currentScreen instanceof WinGameScreen) {
            ...
            if (player.world.getDimensionKey() == World.THE_END) {
                ...
                Biome.Category biome$category = player.world.getBiome(player.getPosition()).getCategory();
                ...
                if (!this.musicTicker.isBackgroundMusicPlaying(BackgroundMusicTracks.UNDER_WATER_MUSIC)
                    ...
                    return player.world.getDimensionKey() != World.THE_NETHER
                            && player.abilities.isCreativeMode && player.abilities.allowFlying
                            ? BackgroundMusicTracks.CREATIVE_MODE_MUSIC
                            : this.world.getBiomeManager().getBiomeAtPosition(player.getPosition())
    ...
    public boolean isEntityGlowing(Entity entity) {
        return entity.isGlowing() || player != null && player.isSpectator()
