...
import im.rarity.functions.settings.impl.*;
...
public class Config implements IMinecraft {
    ...
    public Config(String name) {
        this.name = name;
        this.file = new File(new File(Minecraft.getInstance().gameDir, "\\rarity\\configs"), name + ".cfg");
    }
    ...
    private void loadStyleSettings(JsonObject stylesObject) {
        for (Map.Entry<String, JsonElement> entry : stylesObject.entrySet()) {
            ...
            if (style != null && styleObject.has("selected")) {
                ...
                if (isSelected) {
                    Rarity.getInstance().getStyleManager().setCurrentStyle(style);
                }
            }
        }
    }
    ...
    private Style findStyleByName(String styleName) {
        for (Style style : Rarity.getInstance().getStyleManager().getStyleList()) {
    ...
    private void loadFunctionSettings(JsonObject functionsObject) {
        Rarity.getInstance().getFunctionRegistry().getFunctions().forEach(f -> {
    ...
    private void saveFunctionSettings(JsonObject functionsObject) {
        Rarity.getInstance().getFunctionRegistry().getFunctions().forEach(module -> {
    ...
    private void saveStyleSettings(JsonObject stylesObject) {
        for (Style style : Rarity.getInstance().getStyleManager().getStyleList()) {
            ...
            styleObject.addProperty("selected", Rarity.getInstance().getStyleManager().getCurrentStyle() == style);
